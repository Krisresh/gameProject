class GameScene extends Phaser.Scene {
    ...
    restartMove() {
        this.firstBall.setVelocity(0).setPosition(config.width / 2, config.height / 2);
    ...
    createObjects() {
        ...
        this.input.on("pointermove", (pointer) => {
            const ballPosition = new Phaser.Math.Vector2(this.firstBall.x, this.firstBall.y);
            const pointerPosition = new Phaser.Math.Vector2(pointer.x, pointer.y);
            const lineLength = Phaser.Math.Distance.BetweenPoints(ballPosition, pointerPosition);

            this.line.setTo(ballPosition.x, ballPosition.y, pointerPosition.x, pointerPosition.y);
